        ...
        <div className="w-full max-w-md space-y-6">
          ...
          {otos.length > 0 && (
            <div className="bg-gray-50 rounded-2xl p-5 space-y-3">
              <h3 className="font-semibold text-gray-900 text-sm">{txt.resumoTitulo}</h3>
          ...
          {/* 5. O que vai chegar no email */}
          <div className="bg-purple-50 rounded-2xl p-5 space-y-3">
            <h3 className="font-semibold text-purple-900 text-sm">📬 {txt.emailTitulo}</h3>
            <div className="space-y-2.5">
              {txt.emailItems.map((item, i) => (
          ...
          <div className="space-y-3">
            <h3 className="font-semibold text-gray-900 text-sm">{txt.proximosPassosTitulo}</h3>
            <div className="space-y-2">
              {[
                { emoji: "✅", text: txt.proximosPasso1, done: true },
                { emoji: "📧", text: txt.proximosPasso2, done: false },
                { emoji: "📱", text: txt.instalarAppTitulo, done: false, action: true },
                { emoji: "🎨", text: txt.proximosPasso3, done: false },
              ].map((step, i) => (
          ...
          {showPWA && (
            <div className="bg-gray-50 rounded-2xl p-5 space-y-4 animate-fade-in">
              <h4 className="font-semibold text-gray-800 text-sm">{txt.instalarAppTitulo}</h4>
              <div className="space-y-3">
                <div className="space-y-1.5">
                  <p className="text-xs font-bold text-gray-600 uppercase">iPhone (Safari)</p>
                  <p className="text-sm text-gray-700">{txt.instalarIphone}</p>
                </div>
                <div className="border-t border-gray-200" />
                <div className="space-y-1.5">
                  <p className="text-xs font-bold text-gray-600 uppercase">Android (Chrome)</p>
                  <p className="text-sm text-gray-700">{txt.instalarAndroid}</p>
                </div>
              </div>
            </div>
          )}
